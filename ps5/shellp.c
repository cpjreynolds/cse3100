            ...
            if (strcmp(sc, "cd") == 0) {
                ...
                if (*sc) {
                    if (strchr(ec, '|') != NULL) {
                        ...
                        return setCommandArgs(setCommand(c, basicCMD, args[0]),
                                              nba, args);
                    }
                }
            }
...
// ================================================================================
// This part of the file contains the PipelineCommand object that you must
// modify
// ================================================================================
Stage* allocStage(int nba, char** args)
{
...
int setupCommandPipeline(Command* c)
{
    ...
    // spawnStage: forks a child process, replaces the stdin and stdout with
    // fdin and fdout respectively, and executes the command for the pipeline
...
{
    // consider: which fds open now, which to CLOEXEC.
    // 0 stdin
    // 1 stdout
    // 2 stderr
    // 3.. pipes
    ...
    if (child == 0) {
        ...
        if (mode & R_INP) {
            ...
            assert(dup2(ifd, STDIN_FILENO) == STDIN_FILENO);
        }
        if (mode & (R_OUTP | R_APPD)) {
            int oflags = O_WRONLY | O_CREAT;
            if (mode & R_APPD) {
                oflags |= O_APPEND;
            }
            ofd = open(output, oflags, 0666); // 0666 == -rw-rw-rw
            assert(dup2(ofd, STDOUT_FILENO) == STDOUT_FILENO);
        }
        return execvp(com, args); // does not return.
        assert(0);
    }
    return waitpid(child, 0, 0);
}
